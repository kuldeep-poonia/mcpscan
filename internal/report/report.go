	...
	if len(httpServers) > 0 {
		...
		for _, srv := range httpServers {
			...
			if srv.MCPConfidence == types.ConfidenceConfirmed {
				if len(srv.DangerousParams) > 0 {
					var paramSummaries []string
					for _, dp := range srv.DangerousParams {
						paramSummaries = append(paramSummaries, fmt.Sprintf("%s.%s", dp.ToolName, dp.ParamName))
					}
					joined := strings.Join(paramSummaries, ", ")
					if len(joined) > 30 {
						joined = joined[:27] + "..."
					}
					dangerStr = fmt.Sprintf("%d flagged (%s)", len(srv.DangerousParams), joined)
				} else {
					dangerStr = "0 flagged (safe)"
				}
			}
